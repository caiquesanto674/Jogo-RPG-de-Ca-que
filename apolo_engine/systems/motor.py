...
class Engine_APOLO:
    def __init__(self, owner: str, log_level=LogLevel.INFO):
        self.owner = owner
        self.log = LogSistema(level=log_level)
    ...
    def turno_completo(self):
        ...
        self.log.registrar(
            "PODER", "HIERARQUIA", f"FB Total: {forca_total:.2f}", level=LogLevel.DEBUG
        )
        ...
        self.log.registrar("IA", self.npc_adversario.nome, frase_npc, level=LogLevel.INFO)
        ...
        self.log.registrar(
            "PROTOCOLO", "SHA-256", f"Código: {codigo_sha}", level=LogLevel.DEBUG
        )
